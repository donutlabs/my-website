...
(function(){
	...
	$('.chrismenu .arrow').on("click", function (){
	...
	$(window).on('scroll', function (){
		if($(window).width()>1000){
			if($(window).scrollTop()>200+t){
				$('.chrismenu').removeAttr('style').addClass('pin');
			}else{

				$('.chrismenu').css({top:-$(window).scrollTop()}).removeClass('pin');
			}if($(window).scrollTop()>150+t){
				$('.chrismenu').addClass('before');
			}else{

				$('.chrismenu').removeClass('before');
			}

		}else{

			//$('.chrismenu').css({top:$(window).scrollTop()})
			if($(window).scrollTop()<k){
				$('.chrismenu').addClass('off').removeClass('woff').removeAttr('style');
				$('#menu').removeClass('in');
				k=0;
			}
		}
		if($(window).scrollTop()>t){
			if(!$('.chrismenu').hasClass('woff')){
				$('.chrismenu').addClass('pin-start').addClass('off');
			}

		}else{
			$('.chrismenu').removeClass('pin-start').removeClass('off');
		}
	});
	if($(window).scrollTop()>150+t){
		$('.chrismenu').addClass('pin');
	}else{
		$('.chrismenu').removeAttr('style').removeClass('pin');
	}
	$(window).on("resize", function () {
		if($(window).width()>1000){
			$('.chrismenu').removeAttr('style');
		}
	});
	if($(window).scrollTop()>t){
		$('.chrismenu').addClass('off').addClass('pin-start');
	}else{
		$('.chrismenu').removeClass('off').removeClass('pin-start');
	}
	$('.menu-icon').on("click", function (){
		if($('#menu').hasClass('in')){
			...
			k=$(window).scrollTop();
			$('.chrismenu').removeClass('off').addClass('woff').css({top:$(window).scrollTop()});
		}
	})
